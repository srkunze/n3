...
function TokenStream(debug_stream)
{
	...
	{
		this.debug = true;
		this.on('debug', function(debug){debug_stream.write(debug + '\n');});
	}
};
...
TokenStream.prototype.write = function(buffer)
{
	...
	if (this.debug)
	{
		this.emit('debug', tokens.toString());
	}
	this.emit('data', tokens);
	if (!tokens.length && this.writable)
	{
		this.writable = false;
		this.readable = false;
		this.emit('error');
		this.emit('debug', 'Wrong symbol: ' + this.buffer.substr(this.buffer_index, 10));
	}
};
