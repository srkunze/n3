...
QuadStream.prototype.write = function(tokens)
{
	for (var token_index = 0; token_index < tokens.length; token_index += 1)
	{
		...
		if (!desc) this.emit_error('unexpected token >>' + token.lexem + '<< in state >>' + JSON.stringify(this.contexts, null, 1) + '<<');
		...
		for (var index = 0; index < cmds.length; index += 1)
		{
			...
			else if (cmd == '%blank_end' || cmd == '%graph_end' || cmd == '%list_end')
			{
				...
				if (this.contexts.length == 0) this.emit_error('too many parentheses/brackets/braces closed');
...
{
	this.write(tokens || []);
	if (this.contexts.length > 1 || !this.state['}']) this.emit_error('unexpected end of token stream');
	this.emit('end');
};
